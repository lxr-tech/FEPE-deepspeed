...
class LlamaRotaryEmbedding(torch.nn.Module):
    def __init__(self, dim, n_head, device=None):
        super().__init__()
        # omega = np.linspace(0, 1, dim // 2).reshape((1, -1))
        # omega = torch.cat([torch.FloatTensor(omega), ] * 2, dim=-1).to(device)
        # betas = torch.FloatTensor(np.linspace(1/3, 2, n_head).reshape((-1, 1)) * 0.1).to(device)  # ()
        # self.register_buffer("betas", betas.reshape((-1, )), persistent=False)
        # self.register_buffer("omega", omega * betas*3, persistent=False)
        # self.register_buffer("expos", torch.sqrt((1/betas * torch.exp(-omega/betas)) / (1 / (3*betas))),
        #                      persistent=False)
        # beta = 0.10856
        # omega = np.linspace(np.log(dim) / np.log(10000) / dim, 10000 ** (-1/dim), dim)
        # omega = - np.log(1 - omega).reshape((1, -1)) * beta
        # expos = (beta/omega) / (1/beta * np.exp(-omega/beta))
        # cutoff = 2 * np.pi / (beta / dim)
        # bias = np.mean((beta / omega) / (1 / beta * np.exp(-omega / beta)))
        order = 3
        beta = 1 / np.log(10000)  # 0.10861
        ...
        # cutoff = 2 * np.pi / (2 * beta / np.power(dim, 1.25))
        ...
        # self.register_buffer("cutoff", torch.tensor(cutoff).to(device), persistent=False)
...
def apply_rotary_pos_emb(q, k, cos, sin, expos):
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_num=0):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = LlamaRotaryEmbedding(dim=self.head_dim, n_head=self.num_heads)
        # self.dropout = nn.Dropout(p=0.15)  # todo: add dropout=0.15 (+1)
    ...
    def forward(
        ...
        t = torch.linspace(0, q_len-1, q_len, device=query_states.device).reshape(-1, 1)

        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, expos)
        # [bsz, nh, t, hd], query_states_

        # query_states = query_states * torch.log(t+1)[None, None, :, :]

        # # if not self.training:
        # # scale = self.rotary_emb.cutoff * 3
        # # rand = torch.randn_like(t) * t/512 * 0.05 if self.training else torch.zeros_like(t)  # / torch.exp(rand)
        # # scale = torch.exp((t - q_len // 2) / scale)
        ...
        key_states = key_states / scale[None, None, :, :]

        # todo: add (exp)+log trick
        # order = torch.clamp(t / 4096, min=1 / 2) - q_len / 8192
        # scale = torch.arange(0, self.head_dim, 2, device=query_states.device, dtype=query_states.dtype)
        # scale = torch.clamp_min(10000 ** (scale / self.head_dim) * (math.pi / 2), min=2048)
        # shift = torch.linspace(1, q_len, q_len, device=query_states.device, dtype=query_states.dtype)
        # scale = torch.log(torch.sigmoid(shift / 2048 - 1))
        # scale = torch.cat((self.rotary_emb.inv_freq, self.rotary_emb.inv_freq), dim=-1)
        # scale = (torch.linspace(0, self.head_dim, self.head_dim // 2, device=query_states.device)
        #          + 0.4 * self.head_dim) / (1.4 * self.head_dim)
        # scale = torch.cat((scale, scale), dim=-1)
        # power = (torch.linspace(1, q_len, q_len, device=query_states.device) - q_len // 2) / 2048
        # scale = scale[None, :] ** power[:, None]
        # if not self.training:
        #     query_states = query_states * scale[None, None, :, :]
        #     key_states = key_states / scale[None, None, :, :]
        ...
        # if not self.training:
        # scale = (self.head_dim - torch.linspace(0, self.head_dim, self.head_dim // 2, device=query_states.device)
        #          + 0.4 * self.head_dim) / (1.4 * self.head_dim)
        # scale = torch.cat((scale, scale), dim=-1)
        # power = (torch.linspace(1, q_len, q_len, device=query_states.device) - q_len // 2) / 512
        # scale = scale[None, :] ** power[:, None]
        # scale = scale[:, None] ** ((t - q_len / 2) / 1024)[None, :]
        # scale = (torch.linspace(0, self.head_dim, self.head_dim // 2, device=query_states.device)
        #          + 0.4 * self.head_dim) / (1.4 * self.head_dim)
        # scale = torch.cat((scale, scale), dim=-1)
        # scale = scale[None, :] ** (-(t - q_len / 2) / 1024)[:, None]
        # power = (torch.linspace(1, q_len, q_len, device=query_states.device) - q_len // 2) / 2048
        # scale = math.e ** ((t - q_len / 2) / 1024)[:, None]
        # scale = math.e ** ((self.rotary_emb.omega * 2 / math.pi)[:, None, :] * ((t - q_len / 2) / 1024)[None, :, None])
        # query_states = query_states * scale[None, None, :, :]
        # key_states = key_states / scale[None, None, :, :]  #
